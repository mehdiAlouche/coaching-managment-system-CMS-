  ...
  async (req: Request, res: Response) => {
    try {
      ...
      ActivityService.logActivity(
        'PAYMENT_GENERATED',
        `Payment generated for coach ${coach.firstName || coach.email} - Amount: ${finalAmount} ${currency || 'USD'}`,
        {
          organizationId: orgId,
      ...
      ).catch((error) => console.error('Failed to log PAYMENT_GENERATED activity:', error));
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    ActivityService.logActivity(
      'PAYMENT_GENERATED',
      `Payment generated for coach ${coach.firstName || coach.email} - Amount: ${amount} USD`,
      {
        organizationId: orgId,
        paymentId: payment._id,
        amount,
