...
router.patch(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    // Log activity for session status change
    const activityType = status === 'completed' ? 'SESSION_COMPLETED' : status === 'cancelled' ? 'SESSION_CANCELLED' : undefined;
    if (activityType) {
      ...
      ).catch((error) => console.error(`Failed to log ${activityType} activity:`, error));
    }
