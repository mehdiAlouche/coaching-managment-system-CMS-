  ...
  async (req: Request, res: Response) => {
    try {
      ...
      ActivityService.logActivity(
        'SESSION_CREATED',
        `Session created with ${coach.firstName || coach.email} and ${entrepreneur.firstName || entrepreneur.email}`,
        {
          organizationId: orgId,
          userId,
          ...
          metadata: {
            ...
            entrepreneurName: `${entrepreneur.firstName} ${entrepreneur.lastName}`,
            scheduledAt: scheduledAtDate,
          },
        }
      ).catch((error) => console.error('Failed to log SESSION_CREATED activity:', error));
