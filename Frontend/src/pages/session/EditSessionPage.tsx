...
export default function EditSessionPage({ id }: EditSessionPageProps) {
  ...
  type AgendaItem = Session['agendaItems'] extends Array<infer T> ? T : { title?: string; description?: string; duration?: number }
  ...
  useEffect(() => {
    if (session) {
      setFormData({
        ...
        agendaItems: (session.agendaItems || []).map((item: AgendaItem) => ({
          title: item.title || "",
